...
export default function RavintolaMuokkaus() {
   ...
   useEffect(() =>{
      const getRavintolatById = async () => {
        const raflat = await axios.get(Constants.API_ADDRESS + `/ravintolat/${Constants.SAHKOPOSTI} `)
  ...
  */
     ...
     // tällä toimii "http://localhost:8080/ravintolat/MahtiBurgerit@mahti.fi"
      ...
      const updateAPIData = async () => {
          const result = await axios.put(Constants.API_ADDRESS + `/ravintolat/${Constants.SAHKOPOSTI} `, { 
              nimi: nimi,
              osoite: osoite,
              tyyppi: tyyppi,
